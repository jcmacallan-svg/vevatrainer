...
// Press for answer / ultimatum (used when visitor is evasive)
  ...
  const q=[]; let tmr=null; let approach=null;
    ...
    function enqueueVisitor(text, afterShown){
...
function handleSI(intent, raw){
    ...
    if (intent==="ask_surname"){
      ...
      // Keep the register field as full name if we already have it; otherwise, at least insert surname.
      if (si_name && !si_name.value){
        const fullName = [state.visitor?.first, state.visitor?.last].filter(Boolean).join(" ").trim();
        si_name.value = fullName || last;
      }
